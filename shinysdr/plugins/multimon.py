...
class POCSAGDemodulator(gr.hier_block2, ExportedState):
    '''
    Demod and parse POCSAG.
    '''
    def __init__(self, context):
        ...
        def receive(line):
            ...
            log.msg(u'POCSAG: %r' % (line,))
            message = parse_tnc2(line, time.time())
            log.msg(u'   -> %s' % (message,))
            context.output_message(message)
        
        self.__mm_demod = MultimonNGDemodulator(
            multimon_demod_args=['-a', 'POCSAG1200', '-a', 'POCSAG512'],
            protocol=POCSAGProcessProtocol(receive),
            context=context)
...
class POCSAGProcessProtocol(ProcessProtocol):
...
# TODO: Eliminate this class and replace it with adapters available to any demodulator
class FMPOCSAGDemodulator(gr.hier_block2, ExportedState):
    implements(IDemodulator)
    ...
    def __init__(self, mode, input_rate=0, context=None):
        ...
        # Subprocess
        self.mm_demod = POCSAGDemodulator(context=context)
        mm_audio_rate = self.mm_demod.get_input_type().get_sample_rate()
...
pluginDef_POCSAG = ModeDef(
    mode='POCSAG',  # TODO: Rename mode to be more accurate
    label='POCSAG',
    demod_class=FMPOCSAGDemodulator,
    available=_multimon_available)
